...
# 연결 포트
SLAVE_PORT_CON = '/dev/ttyACM0'
# 통신 버전
SLAVE_PROTOCOL_VERSION = 2.0
# 통신 속도
SLAVE_BAUDRATE = 57600
# End Effector 아이디
END_EFFECTOR = 5
# 연결 아이디
SLAVE_DXL_IDS = [0, 1, 2, 3, 4, END_EFFECTOR]
# 제한 위치 값 
SLAVE_POS_LIMITS =  [[0, 4095],  # Base Rotation     제한 걸어두면 안됌
...
portHandler = PortHandler(SLAVE_PORT_CON)
packetHandler = PacketHandler(SLAVE_PROTOCOL_VERSION)

if not portHandler.openPort():
    raise IOError(f"Failed to open port: {SLAVE_PORT_CON}")

if not portHandler.setBaudRate(SLAVE_BAUDRATE):
    raise IOError(f"Failed to set baudrate: {SLAVE_BAUDRATE}")

for idx, dxl_id in enumerate(SLAVE_DXL_IDS):
    ...
    min_pos, max_pos = SLAVE_POS_LIMITS[idx]
